...
import { createScheduledJobAction, createTeamMeetingAction, deleteCalendarItemAction, rescheduleJobAction } from "./actions";
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        ...
        <details className="group rounded-2xl border border-violet-400/30 bg-violet-500/[0.05] p-3">
          ...
          <form action={createTeamMeetingAction} className="mt-3 grid gap-2 sm:grid-cols-2"><input type="hidden" name="week" value={weekStart}/><input required name="title" placeholder="Meeting title" className={`${field} sm:col-span-2`}/><input required name="date" type="date" defaultValue={defaultDate} className={field}/><div className="grid grid-cols-2 gap-2"><select required name="start" defaultValue="10:00" className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select><select required name="end" defaultValue="10:30" className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select></div><input name="location" placeholder="Office / phone / video" className={field}/><input name="attendees" placeholder="Whole team or names" className={field}/><button className="min-h-10 rounded-lg border border-violet-300/40 bg-violet-500/10 px-4 py-2 text-sm font-semibold text-white sm:col-span-2">Save Meeting</button></form>
        </details>
      </div>

      <section className="overflow-x-auto rounded-2xl border border-[#2d7dff]/25 bg-black/45 p-2">
        <div className="grid min-w-[980px] grid-cols-7 gap-2">{days.map(day=>{ const dayJobs=scheduledThisWeek.filter(e=>e.slot?.date===day).sort((a,b)=>(a.slot?.start??"").localeCompare(b.slot?.start??"")); return <div key={day} className="min-h-[240px] rounded-xl border border-[#2d7dff]/15 bg-zinc-950/65 p-2"><div className="mb-2 border-b border-[#2d7dff]/15 pb-2"><p className="text-xs font-semibold text-white">{labelDate(day)}</p><p className="text-[10px] text-zinc-500">{dayJobs.length} scheduled</p></div><div className="space-y-1.5">{dayJobs.length===0?<p className="rounded-lg border border-dashed border-[#2d7dff]/15 p-2 text-[11px] text-zinc-600">Open</p>:dayJobs.map(({job,slot})=>{ const meeting=isMeeting(job.scope); return <article key={job.id} className={`rounded-lg border p-2 ${meeting?"border-violet-400/30 bg-violet-500/[0.06]":"border-[#2d7dff]/20 bg-black/55"}`}><p className="text-[11px] font-semibold text-[#8ffafa]">{displayTime(slot!.start)}–{displayTime(slot!.end)}</p><p className="mt-0.5 truncate text-xs font-semibold text-white">{meeting?meetingTitle(job.scope):job.customerName}</p><p className="truncate text-[11px] text-zinc-400">{job.location || "No location"}</p><p className="truncate text-[11px] text-zinc-500">{meeting?"Team meeting":job.assignedTechName??"Unassigned"}</p><div className="mt-1.5 flex items-center gap-2"><details className="min-w-0 flex-1"><summary className="cursor-pointer text-[11px] font-semibold text-[#d9fbff]">Edit</summary><form action={rescheduleJobAction} className="mt-2 space-y-1.5"><input type="hidden" name="jobId" value={job.id}/><input type="hidden" name="week" value={weekStart}/><input name="date" type="date" defaultValue={slot!.date} className={field}/><div className="grid grid-cols-2 gap-1"><select name="start" defaultValue={slot!.start} className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select><select name="end" defaultValue={slot!.end} className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select></div>{!meeting?<select name="assignedTechId" defaultValue={job.assignedTechId??""} className={field}><option value="">Unassigned</option>{technicians.map(t=><option key={t.id} value={t.id}>{t.fullName}</option>)}</select>:<input type="hidden" name="assignedTechId" value=""/>}<button className="w-full rounded-lg border border-[#2d7dff]/30 px-3 py-2 text-[11px] text-white">Save</button></form></details><form action={deleteCalendarItemAction}><input type="hidden" name="jobId" value={job.id}/><input type="hidden" name="week" value={weekStart}/><button className="rounded-md border border-rose-500/35 px-2 py-1 text-[10px] font-semibold text-rose-200">Delete</button></form></div>{!meeting?<Link href={`/jobs/${job.id}`} className="mt-1.5 block text-[10px] text-zinc-500 underline underline-offset-2">Open call</Link>:null}</article>})}</div></div>})}</div>
      </section>
    </div>
  </AppShell>;
}
