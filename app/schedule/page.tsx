...
const field = "min-h-10 w-full rounded-lg border border-[#2d7dff]/25 bg-black px-3 py-2 text-center text-sm text-white";
    ...
    <div className="space-y-3 text-center">
      <section className="rounded-2xl border border-[#2d7dff]/25 bg-black/45 p-3">
        <div className="flex flex-wrap items-center justify-center gap-2 sm:justify-between">
          <div className="text-center"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#8ffafa]">Week of {labelDate(weekStart)}</p><p className="text-xs text-zinc-500">{scheduledThisWeek.length} scheduled</p></div>
          <div className="flex justify-center gap-2"><Link href={`/schedule?week=${previousWeek}`} className="rounded-lg border border-[#2d7dff]/25 px-3 py-2 text-center text-xs text-[#d9fbff]">Previous</Link><Link href="/schedule" className="rounded-lg border border-[#2d7dff]/25 px-3 py-2 text-center text-xs text-[#d9fbff]">This week</Link><Link href={`/schedule?week=${nextWeek}`} className="rounded-lg border border-[#2d7dff]/25 px-3 py-2 text-center text-xs text-[#d9fbff]">Next</Link></div>
        </div>
        {params.success ? <p className="mt-2 rounded-lg border border-emerald-400/25 bg-emerald-500/10 px-3 py-2 text-center text-xs text-emerald-200">{params.success}</p> : null}
        {params.error ? <p className="mt-2 rounded-lg border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-center text-xs text-rose-200">{params.error}</p> : null}
      </section>

      <div className="grid grid-cols-2 gap-2">
        <details className="group rounded-xl border border-[#8ffafa]/30 bg-[#06111b]/90 p-2 open:col-span-2 sm:p-3">
          <summary className="cursor-pointer list-none text-center text-sm font-semibold text-white">Schedule service call <span className="ml-1 text-[10px] font-normal text-zinc-500 sm:text-xs">tap to expand</span></summary>
          <form action={createScheduledJobAction} className="mt-3 grid gap-2 sm:grid-cols-2"><input type="hidden" name="week" value={weekStart}/><select required name="customerId" defaultValue="" className={field}><option value="">Customer</option>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><select name="assignedTechId" defaultValue="" className={field}><option value="">Unassigned technician</option>{technicians.map(t=><option key={t.id} value={t.id}>{t.fullName}</option>)}</select><input required name="date" type="date" defaultValue={defaultDate} className={field}/><div className="grid grid-cols-2 gap-2"><select required name="start" defaultValue="09:00" className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select><select required name="end" defaultValue="11:00" className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select></div><input name="location" placeholder="Service location" className={field}/><input required name="scope" placeholder="Complaint / scope" className={field}/><button type="submit" className="min-h-10 rounded-lg border border-[#8ffafa]/55 bg-[#2d7dff]/20 px-4 py-2 text-center text-sm font-semibold text-white sm:col-span-2">Save Call</button></form>
        </details>

        <details className="group rounded-xl border border-violet-400/30 bg-violet-500/[0.05] p-2 open:col-span-2 sm:p-3">
          <summary className="cursor-pointer list-none text-center text-sm font-semibold text-white">Schedule team meeting <span className="ml-1 text-[10px] font-normal text-zinc-500 sm:text-xs">tap to expand</span></summary>
          <form action={createTeamMeetingAction} className="mt-3 grid gap-2 sm:grid-cols-2"><input type="hidden" name="week" value={weekStart}/><input required name="title" placeholder="Meeting title" className={`${field} sm:col-span-2`}/><input required name="date" type="date" defaultValue={defaultDate} className={field}/><div className="grid grid-cols-2 gap-2"><select required name="start" defaultValue="10:00" className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select><select required name="end" defaultValue="10:30" className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select></div><input name="location" placeholder="Office / phone / video" className={field}/><input name="attendees" placeholder="Whole team or names" className={field}/><button type="submit" className="min-h-10 rounded-lg border border-violet-300/40 bg-violet-500/10 px-4 py-2 text-center text-sm font-semibold text-white sm:col-span-2">Save Meeting</button></form>
        </details>
      </div>

      <section className="overflow-x-auto rounded-2xl border border-[#2d7dff]/25 bg-black/45 p-2">
        <div className="grid min-w-[900px] grid-cols-7 gap-1.5">{days.map(day=>{ const dayJobs=scheduledThisWeek.filter(e=>e.slot?.date===day).sort((a,b)=>(a.slot?.start??"").localeCompare(b.slot?.start??"")); return <div key={day} className="min-h-[180px] rounded-lg border border-[#2d7dff]/15 bg-zinc-950/65 p-1.5 text-center"><div className="mb-1.5 border-b border-[#2d7dff]/15 pb-1.5"><p className="text-[11px] font-semibold text-white">{labelDate(day)}</p><p className="text-[9px] text-zinc-500">{dayJobs.length} scheduled</p></div><div className="space-y-1">{dayJobs.length===0?<p className="rounded-md border border-dashed border-[#2d7dff]/15 p-1.5 text-center text-[10px] text-zinc-600">Open</p>:dayJobs.map(({job,slot})=>{ const meeting=isMeeting(job.scope); return <article key={job.id} className={`rounded-md border p-1.5 text-center ${meeting?"border-violet-400/30 bg-violet-500/[0.06]":"border-[#2d7dff]/20 bg-black/55"}`}><p className="text-[10px] font-semibold text-[#8ffafa]">{displayTime(slot!.start)}–{displayTime(slot!.end)}</p><p className="truncate text-[11px] font-semibold text-white">{meeting?meetingTitle(job.scope):job.customerName}</p><p className="truncate text-[9px] text-zinc-400">{job.location || "No location"}</p><p className="truncate text-[9px] text-zinc-500">{meeting?"Team meeting":job.assignedTechName??"Unassigned"}</p><div className="mt-1.5 grid grid-cols-2 gap-1.5"><details className="relative z-10 min-w-0 rounded-lg border border-[#2d7dff]/25 bg-black/30"><summary className="cursor-pointer list-none px-2 py-2 text-center text-[11px] font-semibold text-[#d9fbff]">Edit</summary><form action={rescheduleJobAction} className="absolute left-0 top-full z-30 mt-1 w-[260px] space-y-1.5 rounded-xl border border-[#2d7dff]/30 bg-[#03070d] p-2 text-center shadow-2xl"><input type="hidden" name="jobId" value={job.id}/><input type="hidden" name="week" value={weekStart}/><input name="date" type="date" defaultValue={slot!.date} className={field}/><div className="grid grid-cols-2 gap-1"><select name="start" defaultValue={slot!.start} className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select><select name="end" defaultValue={slot!.end} className={field}>{TIMES.map(t=><option key={t} value={t}>{displayTime(t)}</option>)}</select></div>{!meeting?<select name="assignedTechId" defaultValue={job.assignedTechId??""} className={field}><option value="">Unassigned</option>{technicians.map(t=><option key={t.id} value={t.id}>{t.fullName}</option>)}</select>:<input type="hidden" name="assignedTechId" value=""/>}<button type="submit" className="w-full rounded-lg border border-[#2d7dff]/30 px-3 py-2 text-center text-[11px] text-white">Save</button></form></details><ScheduleDeleteButton jobId={job.id} week={weekStart}/></div>{!meeting?<Link href={`/jobs/${job.id}`} className="mt-1 block text-center text-[9px] text-zinc-500 underline underline-offset-2">Open call</Link>:null}</article>})}</div></div>})}</div>
      </section>
    </div>
  </AppShell>;
}