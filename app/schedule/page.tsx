...
type Props = { searchParams: Promise<{ week?: string; success?: string; error?: string; warning?: string; customer?: string }> };
  ...
  return <AppShell title="Scheduling">
    <div className="space-y-3 text-center">
      <section className="panel rounded-2xl p-3">
        ...
        {params.success ? <p className="mt-2 rounded-lg px-3 py-2 text-center text-xs text-emerald-200">{params.success}</p> : null}
        {params.warning ? <p className="mt-2 text-center text-xs text-amber-200">{params.warning}</p> : null}
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <details className="panel group rounded-xl p-2 open:sm:col-span-3 sm:p-3">
          ...
          <form action={createScheduleCustomerAction} className="mt-3 grid gap-2 sm:grid-cols-2" data-draft-key={`schedule:customer:${weekStart}`} data-draft-label="New schedule customer" noValidate><input type="hidden" name="week" value={weekStart}/><input required name="name" placeholder="Customer / business name" className={field}/><input name="phone" type="tel" placeholder="Phone" className={field}/><input name="email" type="email" placeholder="Email" className={field}/><input name="address" placeholder="Service / billing address" className={field}/><button type="submit" className="box hot min-h-11 rounded-lg px-4 py-2 text-center text-sm font-semibold text-white sm:col-span-2">Add Customer</button></form>
        </details>
