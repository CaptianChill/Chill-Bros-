  ...
  return <AppShell title="Scheduling">
    <div className="space-y-4">
      <section className="rounded-3xl border border-[#2d7dff]/25 bg-black/45 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8ffafa]">Week of {labelDate(weekStart)}</p><p className="mt-1 text-sm text-zinc-400">{scheduledThisWeek.length} saved calendar item{scheduledThisWeek.length===1?"":"s"}</p></div><div className="flex gap-2"><Link href={`/schedule?week=${previousWeek}`} className="rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-sm text-[#d9fbff]">Previous</Link><Link href="/schedule" className="rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-sm text-[#d9fbff]">This week</Link><Link href={`/schedule?week=${nextWeek}`} className="rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-sm text-[#d9fbff]">Next</Link></div></div>
        <p className="mt-3 text-xs leading-5 text-zinc-500">Service calls are loaded directly from the production schedule records, including older jobs that were rescheduled. Saving a call writes it to the shared Chill Bros calendar used by Dispatch, Office, Customer Center, and technician views.</p>
