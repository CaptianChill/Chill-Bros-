...
export async function createDirectInvoiceAction(formData: FormData): Promise<never> {
  ...
  const [{ data: catalogRows }, { data: feeRows }] = await Promise.all([
    supabase.from("chillbros_parts_catalog").select("id,name,part_number,retail_price,stock"),
  ...
  const lines: DirectLine[] = Array.from({ length: 8 }, (_, i) => {
    ...
    let inventoryPartId: string | null = null;
    if (preset.startsWith("part:")) {
      const row = catalog.get(preset.slice(5));
      if (row && !String(row.part_number ?? "").startsWith("PB-")) {
        presetLabel = row.name;
        presetDescription = row.part_number || "Inventory part";
        presetPrice = Number(row.retail_price ?? 0);
        inventoryPartId = String(row.id);
      }
  ...
  for (const row of lines) {
    if (row.quantity <= 0 || row.unit_price < 0 || row.label.length > 200) fail("Check each line item, quantity, and price.", type);
    if (row.inventoryPartId && (!Number.isInteger(row.quantity) || row.quantity > 100)) fail("Inventory quantities must be whole units between 1 and 100 per invoice line.", type);
  }
  ...
  const { data: job, error: jobError } = await supabase.from("chillbros_jobs").insert({
    customer_id: customerId, assigned_tech_id: profile.id, status: "scheduled", location: jobLocation,
    scope: scope.slice(0,4000), work_performed: text(formData,"workPerformed").slice(0,4000) || null,
    scheduled_window: `Standalone ${type} · internal billing record`,
  }).select("id").single();
  if (jobError || !job) fail(jobError?.message ?? `Could not create standalone ${type}.`, type);

  const inventoryByPart = new Map<string, number>();
  if (type === "invoice") {
    for (const row of lines) {
      if (!row.inventoryPartId) continue;
      inventoryByPart.set(row.inventoryPartId, (inventoryByPart.get(row.inventoryPartId) ?? 0) + row.quantity);
    }
  }

  const allocatedJobPartIds: string[] = [];
  const rollbackInventory = async () => {
    for (const jobPartId of allocatedJobPartIds.reverse()) {
      try { await supabase.rpc("chillbros_set_job_part_quantity", { p_job_part_id: jobPartId, p_quantity: 0 }); } catch { /* best-effort rollback */ }
    }
  };

  for (const [partId, quantity] of inventoryByPart) {
    if (!Number.isInteger(quantity) || quantity < 1 || quantity > 100) {
      ...
      fail("Combined inventory quantity for one part must be between 1 and 100 units.", type);
    }
    const { data: jobPartId, error: stockError } = await supabase.rpc("chillbros_add_job_part", {
      p_job_id: job.id,
      p_part_id: partId,
      p_quantity: quantity,
    });
    if (stockError || !jobPartId) {
      await rollbackInventory();
      await supabase.from("chillbros_jobs").update({ status: "completed", updated_at: new Date().toISOString() }).eq("id", job.id);
      fail(stockError?.message === "insufficient stock" ? "Not enough inventory is available for one of the selected parts." : stockError?.message ?? "Could not allocate inventory to this invoice.", type);
    }
  ...
  const { data: created, error: createError } = await supabase.rpc("chillbros_create_estimate_v2", {
    ...
    p_line_items: lines.map(({ inventoryPartId: _inventoryPartId, ...line }) => line),
  ...
  if (createError || !created) {
    await rollbackInventory();
    await supabase.from("chillbros_jobs").update({ status: "completed", updated_at: new Date().toISOString() }).eq("id", job.id);
    fail(createError?.message ?? `Could not create ${type}.`, type);
  }
