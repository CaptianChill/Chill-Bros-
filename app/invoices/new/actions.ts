...
export async function createDirectInvoiceAction(formData: FormData): Promise<never> {
  ...
  for (const [partId, quantity] of inventoryByPart) { if (!Number.isInteger(quantity) || quantity < 1 || quantity > 100) { await rollbackInventory(); await supabase.from("chillbros_jobs").update({ status: "completed", updated_at: new Date().toISOString() }).eq("id", job.id); fail("Combined inventory quantity for one part must be between 1 and 100 units.", type); } const { data: jobPartId, error: stockError } = await supabase.rpc("chillbros_add_job_part", { p_job_id: job.id, p_part_id: partId, p_quantity: quantity }); if (stockError || !jobPartId) { await rollbackInventory(); await supabase.from("chillbros_jobs").update({ status: "completed", updated_at: new Date().toISOString() }).eq("id", job.id); fail(stockError?.message === "insufficient stock" ? "Not enough inventory is available for one of the selected parts." : stockError?.message ?? "Could not allocate inventory to this invoice.", type); } allocatedJobPartIds.push(String(jobPartId)); }
