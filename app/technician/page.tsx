    ...
    <AppShell
      ...
      highlight={
        <div className="space-y-3">
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">Assigned job</p>
            ...
            <div className="rounded-2xl border border-cyan-400/20 bg-black/40 p-4">
              ...
              <div className="mt-4 space-y-3">
                {serviceJob.partsUsed.map((part) => (
                  <div key={part.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-cyan-400/10 bg-zinc-950/80 px-4 py-3">
                    <div>
                      <p className="text-white">{part.name}</p>
                      <p className="text-sm text-zinc-400">{part.partNumber}</p>
                    </div>
                    <p className="text-cyan-200">${part.retailPrice}</p>
                  </div>
                ))}
              </div>
            </div>
