...
import { CalendarDays, ClipboardList, Mail, MapPin, UserRound } from "lucide-react";
      ...
      <div className="space-y-4">
        <SectionCard title="Today">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-sm text-zinc-300"><CalendarDays className="h-4 w-4 text-[#8ffafa]" /><span>{todaySchedule.length} scheduled item{todaySchedule.length === 1 ? "" : "s"}</span></div>
            <Link href="/schedule" className="rounded-lg border border-[#2d7dff]/25 px-3 py-2 text-xs font-semibold text-[#d9fbff]">Open Schedule</Link>
          </div>
          {todaySchedule.length === 0 ? (
            ...
            <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              {todaySchedule.map(({ job, slot }) => (
                <Link key={job.id} href="/schedule" className="rounded-xl border border-[#2d7dff]/20 bg-black/40 p-3 transition hover:border-[#8ffafa]/40 hover:bg-[#2d7dff]/10">
                  <div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-[#8ffafa]">{displayTime(slot!.start)}</p><p className="text-[10px] uppercase tracking-[0.12em] text-zinc-500">{job.status.replace(/_/g," ")}</p></div>
                  <p className="mt-1 truncate text-sm font-semibold text-white">{job.customerName}</p>
                  <p className="mt-1 flex items-center gap-1 truncate text-xs text-zinc-400"><MapPin className="h-3 w-3 shrink-0" />{job.location || "No location"}</p>
                  <p className="mt-1 flex items-center gap-1 truncate text-xs text-zinc-500"><UserRound className="h-3 w-3 shrink-0" />{job.assignedTechName || "Unassigned"}</p>
                </Link>
              ))}
            </div>
          )}
