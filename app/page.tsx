      ...
      <div className="space-y-4">
        <SectionCard eyebrow="Core workflow" title="One path from call to cash" description="These are the operating screens. Everything else stays out of the way until the core system is stable.">
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            <Link href="/schedule" className="rounded-xl border border-[#2d7dff]/25 bg-black/40 p-3 transition hover:border-[#8ffafa]/45 hover:bg-[#2d7dff]/10"><CalendarDays className="mx-auto h-4 w-4 text-[#8ffafa]" /><p className="mt-2 text-sm font-semibold text-white">1. Schedule</p><p className="mt-1 text-[11px] text-zinc-500">Book the call</p></Link>
            <Link href="/dispatch" className="rounded-xl border border-[#2d7dff]/25 bg-black/40 p-3 transition hover:border-[#8ffafa]/45 hover:bg-[#2d7dff]/10"><Route className="mx-auto h-4 w-4 text-[#8ffafa]" /><p className="mt-2 text-sm font-semibold text-white">2. Dispatch / Job</p><p className="mt-1 text-[11px] text-zinc-500">Assign and work it</p></Link>
            <Link href="/customers" className="rounded-xl border border-[#2d7dff]/25 bg-black/40 p-3 transition hover:border-[#8ffafa]/45 hover:bg-[#2d7dff]/10"><UsersRound className="mx-auto h-4 w-4 text-[#8ffafa]" /><p className="mt-2 text-sm font-semibold text-white">3. Customer</p><p className="mt-1 text-[11px] text-zinc-500">Keep one record</p></Link>
            <Link href="/invoices" className="rounded-xl border border-[#2d7dff]/25 bg-black/40 p-3 transition hover:border-[#8ffafa]/45 hover:bg-[#2d7dff]/10"><Banknote className="mx-auto h-4 w-4 text-[#8ffafa]" /><p className="mt-2 text-sm font-semibold text-white">4. Quote / Invoice</p><p className="mt-1 text-[11px] text-zinc-500">Approve and bill</p></Link>
            <Link href="/payments" className="rounded-xl border border-[#2d7dff]/25 bg-black/40 p-3 transition hover:border-[#8ffafa]/45 hover:bg-[#2d7dff]/10"><CreditCard className="mx-auto h-4 w-4 text-[#8ffafa]" /><p className="mt-2 text-sm font-semibold text-white">5. Payment</p><p className="mt-1 text-[11px] text-zinc-500">Close the loop</p></Link>
          </div>
        </SectionCard>

        <SectionCard title="Needs attention">
          {attention.length === 0 ? (
            <div className="flex items-center justify-center gap-3 rounded-xl border border-emerald-400/20 bg-emerald-400/[0.05] p-4 text-sm text-emerald-100"><CheckCircle2 className="h-5 w-5" />No workflow handoffs are waiting on you.</div>
          ) : (
        ...
        <SectionCard title="Customers">
          <details className="rounded-xl border border-[#2d7dff]/20 bg-black/40">
            ...
            <div className="max-h-72 overflow-y-auto border-t border-[#2d7dff]/15 p-2">{customers.length === 0 ? <p className="p-2 text-sm text-zinc-500">No customers yet.</p> : customers.map((customer) => <Link key={customer.id} href={`/customers/${customer.id}`} className="flex items-center justify-between gap-3 rounded-lg px-3 py-2 text-sm transition hover:bg-[#2d7dff]/10"><span className="truncate text-white">{customer.name}</span><span className="truncate text-xs text-zinc-500">{customer.address ?? "No address"}</span></Link>)}</div>
          </details>
        </SectionCard>
      </div>
    </AppShell>
