    ...
    <AppShell
      ...
      highlight={
        <div className="space-y-3">
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">CRM pulse</p>
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <SectionCard eyebrow="Customer directory" title="Active customer records" description="Store names, addresses, phone numbers, emails, and recent service history in one clean list.">
          <div className="space-y-4">
            {customerDirectory.map((customer) => (
              <div key={customer.id} className="rounded-2xl border border-cyan-400/20 bg-black/40 p-4">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl border border-cyan-400/10 bg-zinc-950/80 p-3 text-sm text-zinc-300">
                    <p>{customer.phone}</p>
                    <p className="mt-1 text-cyan-200">{customer.email}</p>
                  </div>
                  <div className="rounded-2xl border border-cyan-400/10 bg-zinc-950/80 p-3 text-sm text-zinc-300">
        ...
        <SectionCard eyebrow="Email hub" title="Outbound communications" description="Quotes, invoices, and receipts stay visible here so the office can confirm delivery and follow-up.">
          <div className="space-y-3">
            {emailLog.map((entry) => (
              <div key={entry.subject} className="rounded-2xl border border-cyan-400/20 bg-black/40 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
