    ...
    <AppShell
      ...
      highlight={
        <div className="space-y-3">
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">Catalog state</p>
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <SectionCard eyebrow="Parts database" title="Searchable service inventory" description="Create, review, and price standard parts so technician quotes and customer invoices stay consistent.">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-cyan-400/30 px-4 py-2 text-sm text-cyan-100">
            <Search className="h-4 w-4" />
            Quick search lookups available while filling service tickets
          </div>
          <div className="overflow-hidden rounded-3xl border border-cyan-400/20 bg-black/40">
            <table className="min-w-full text-left text-sm text-zinc-300">
              ...
              <tbody>
                {partsCatalog.map((part) => (
                  <tr key={part.id} className="border-t border-cyan-400/10">
                    ...
                    <td className="px-4 py-3 text-cyan-200">${part.retailPrice}</td>
        ...
        <SectionCard eyebrow="Auto fees" title="Baseline service charges" description="Managers can keep dispatch and arrival pricing aligned so new jobs open with the correct defaults.">
          <div className="space-y-3">
            {feeSettings.map((fee) => (
              <div key={fee.label} className="rounded-2xl border border-cyan-400/20 bg-black/40 p-4">
            ...
            <div className="rounded-2xl border border-dashed border-cyan-400/30 bg-black/20 p-4 text-sm text-zinc-300">
