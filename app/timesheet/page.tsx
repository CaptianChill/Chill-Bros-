...
export default async function TimesheetPage() {
  ...
  if (profile.role === "manager") {
    ...
    return (
      <AppShell title="Review staff clock records, labor hours, and drive time." description="Manager timesheet history shows recent field and office sessions in Central Time with labor and drive allocation." highlight={<div className="space-y-3"><p className="text-sm uppercase tracking-[0.3em] text-[#8ffafa]">Timesheet review</p><StatusPill tone="emerald">{rows.length} sessions</StatusPill><StatusPill>{totalHours.toFixed(1)} recorded hours</StatusPill></div>}>
        <SectionCard eyebrow="Manager review" title="Team time history" description="Recent clock sessions, newest first.">
  ...
  return (
    <AppShell title={isOffice ? "Track the office workday from clock-in through breaks and clock-out." : "Clock in once, refresh safely, and close the shift with labor and drive time."} description={isOffice ? "Office / Dispatch staff can track the full day and lunch breaks while the session stays safely stored in Supabase." : "Open clock sessions are restored from Supabase, so closing the browser or refreshing no longer loses the active timesheet."} highlight={<div className="space-y-3"><p className="text-sm uppercase tracking-[0.3em] text-[#8ffafa]">{isOffice ? "Office time" : "Field time"}</p><StatusPill tone={open ? "emerald" : "cyan"}>{open ? "Open session restored" : "Ready to clock in"}</StatusPill><StatusPill>Central Time / 12-hour display</StatusPill></div>}>
      <SectionCard eyebrow={isOffice ? "Office staff time capture" : "Technician time capture"} title="Daily timesheet" description={isOffice ? "Clock the full office day, punch out and back in for lunch/breaks, and keep the manager timesheet current." : "Clock state persists in the database and links automatically to the assigned active job."}>
