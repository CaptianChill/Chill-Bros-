      ...
      <SectionCard eyebrow="Open / In Progress" title="Saved work for this customer" description="Anything unfinished stays easy to find here. Open a document, continue editing it, or resume a form saved on this device.">
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-2">
            ...
            {openDocuments.length === 0 ? <p className="text-sm text-zinc-500">No unpaid or unfinished billing documents.</p> : openDocuments.map((document) => <div key={document.id} className="rounded-xl border border-[#2d7dff]/15 bg-black/40 p-3">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-3"><Link href={`/portal/${document.portalToken}/document`} target="_blank" className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-[#2d7dff]/25 px-2.5 py-2 text-xs text-[#d9fbff]"><FileText className="h-3.5 w-3.5" />Open</Link><Link href={`/invoices?focus=${encodeURIComponent(document.id)}`} className="inline-flex items-center justify-center rounded-lg border border-[#8ffafa]/35 bg-[#2d7dff]/10 px-2.5 py-2 text-xs font-semibold text-[#d9fbff]">Edit / Continue</Link><Link href={`/portal/${document.portalToken}`} target="_blank" className="inline-flex items-center justify-center rounded-lg border border-[#2d7dff]/25 px-2.5 py-2 text-xs text-[#d9fbff]">Customer view</Link></div>
            </div>)}
          </div>
