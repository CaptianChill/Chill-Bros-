    ...
    <div className="mx-auto w-full max-w-4xl">
      ...
      <article className="w-full max-w-full overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-xl print:rounded-none print:border-0 print:shadow-none">
        ...
        <div className="space-y-6 p-4 sm:p-8">
          ...
          {invoice.status === "approved" && !invoiceIssued && !paid ? <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex items-start gap-3"><BadgeCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700" /><div><p className="font-semibold text-emerald-900">Estimate approved</p><p className="mt-1 text-sm leading-6 text-emerald-900/80">{downPaymentRequired && !downPaymentPaid ? "This signed document authorizes the quoted work. A down payment is due before work begins — see below." : downPaymentRequired ? "Down payment received. The final invoice and remaining balance will appear after Chill Pros completes the approved work." : "This signed document authorizes the quoted work. The final invoice and payment options will appear after Chill Pros completes the approved work."}</p></div></div></section> : null}
