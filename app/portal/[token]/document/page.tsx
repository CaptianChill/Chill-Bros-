    ...
    <div className="mx-auto max-w-4xl">
      ...
      <article className="overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-xl print:rounded-none print:border-0 print:shadow-none">
        ...
        <div className="space-y-6 p-6 sm:p-8">
          ...
          <section className="grid gap-4 border-t border-zinc-200 pt-5 sm:grid-cols-2"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Customer approval</p><p className="mt-2 text-sm">Signed by: <span className="font-semibold">{invoice.signatureName ?? "Pending"}</span></p>{invoice.signedAt ? <p className="mt-1 text-xs text-zinc-500">{new Date(invoice.signedAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</p> : null}</div><div className="sm:text-right"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Payment method</p><p className="mt-2 text-sm font-semibold">{invoice.paymentMethod ? invoice.paymentMethod.replace(/_/g, " ") : "Not selected"}</p></div></section>
        </div>
      </article>
    </div>
  </main>;
