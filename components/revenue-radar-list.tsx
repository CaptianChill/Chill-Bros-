...
export function RevenueRadarList({ prospects }: { prospects: Prospect[] }) {
  ...
  return <section className="space-y-4">
    ...
    <div className="space-y-3">
      {visible.length === 0 ? <div className="rounded-2xl border border-white/15 bg-black/25 p-5 text-center text-sm text-zinc-400">
      ...
      </div> : visible.map((p) => <Link key={p.id} href={"/revenue-radar/" + p.id} className="block rounded-2xl border border-cyan-400/25 bg-black/40 p-4 transition hover:border-cyan-300">
        ...
        <p className="mt-2 text-xs text-zinc-400">{p.signal_verified ? "Verified" : "Auto-discovered public lead"} · {p.status.replaceAll("_", " ")} · {new Date(p.signal_observed_at).toLocaleDateString()}</p>
