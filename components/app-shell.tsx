      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-3 pb-8 pt-2.5 sm:px-6 sm:pt-4 lg:px-8">
        <header className="sticky top-0 z-20 mb-3 rounded-2xl border border-[#2d7dff]/45 bg-[#020407]/94 px-3 py-2.5 shadow-[0_0_16px_rgba(45,125,255,0.36)] backdrop-blur-xl sm:mb-5 sm:rounded-3xl sm:px-4 sm:py-4">
          <div className="flex items-center gap-2.5 sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span className="shrink-0 rounded-full border border-emerald-400/40 bg-emerald-400/10 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-emerald-200 sm:hidden">
                  Live
                </span>
              </div>
            </div>
          ...
          <div className="mt-2 flex items-center justify-between gap-2 border-t border-[#2d7dff]/15 pt-2 text-xs sm:mt-3 sm:flex-wrap sm:border-0 sm:pt-0">
            <div className="hidden items-center gap-2 rounded-full border border-[#2d7dff]/25 bg-black/35 px-3 py-1.5 text-[#d9fbff] sm:inline-flex">
              <Snowflake className="h-3.5 w-3.5" />
              Supabase-backed operations
            </div>

            {profile ? (
              <form action={signOutAction} className="min-w-0 flex-1 sm:flex-none">
                <div className="flex min-w-0 items-center justify-between gap-2 sm:justify-start sm:rounded-full sm:border sm:border-[#2d7dff]/25 sm:bg-black/35 sm:px-3 sm:py-1.5">
                  <span className="truncate text-[11px] text-zinc-300 sm:text-xs">
                    {profile.fullName} · {profile.role === "manager" ? "Manager" : "Technician"}
                  </span>
                  <button type="submit" className="inline-flex shrink-0 items-center gap-1 rounded-full border border-[#2d7dff]/30 bg-[#2d7dff]/5 px-2 py-1 text-[10px] text-[#d9fbff] transition hover:bg-[#2d7dff]/15 sm:text-xs" aria-label="Sign out">
          ...
          <nav className="mt-2 flex gap-1.5 overflow-x-auto pb-0.5 sm:mt-3 sm:gap-2 sm:pb-1">
            {visibleNavItems.map((item) => (
              <Link key={item.href} href={item.href} className="whitespace-nowrap rounded-full border border-[#2d7dff]/30 bg-black/35 px-3 py-1.5 font-brand text-[10px] uppercase tracking-[0.12em] text-[#d9fbff] shadow-[0_0_8px_rgba(45,125,255,0.16)] transition hover:bg-[#2d7dff]/15 sm:px-4 sm:py-2 sm:text-sm">
