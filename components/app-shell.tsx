...
export async function AppShell({ children }: AppShellProps) {
  ...
  return <div className="min-h-screen bg-transparent text-foreground">
    <a href="#main-content" className="box fixed left-3 top-3 z-[100] -translate-y-24 rounded-xl bg-[#8ffafa] px-4 py-2 text-center text-sm font-semibold text-black transition focus:translate-y-0">Skip to content</a>
    <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-3 pb-7 pt-2.5 sm:px-6 sm:pt-4 lg:px-8">
      <header
        className="panel app-header sticky z-40 mb-2.5 rounded-2xl border border-[#2d7dff]/35 bg-[#020407] px-3 py-2.5 text-center shadow-[0_0_14px_rgba(45,125,255,0.24)] sm:mb-3 sm:px-4 sm:py-3"
      ...
      >
        <div className="flex items-center justify-center gap-2.5 sm:gap-4">
          ...
          <div className="min-w-0 flex-1 text-center">
            ...
            <div className="sub mt-1 hidden truncate font-brand text-[0.58rem] font-semibold uppercase tracking-[0.2em] text-[#d9fbff] sm:block">Operational Command Center</div>
          </div>
          <div className="flex shrink-0 items-center justify-center gap-1.5">
            <PageSyncControls />
            {profile ? <form action={signOutAction}><button type="submit" title={`Sign out ${profile.fullName}`} className="box inline-flex min-h-10 items-center justify-center gap-1 rounded-xl border border-[#2d7dff]/25 bg-black/35 px-2.5 py-2 text-center text-[10px] text-[#d9fbff] transition hover:bg-[#2d7dff]/15 sm:text-xs"><LogOut className="h-3.5 w-3.5" /><span className="hidden md:inline">{profile.fullName} · {roleLabel}</span><span className="md:hidden">Exit</span></button></form> : null}
          </div>
        </div>
