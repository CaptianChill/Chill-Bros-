...
export function ServiceAgreementClientActions({ agreement }: { agreement: ServiceAgreement }) {
  ...
  const accept = () => {
    ...
    startTransition(async () => {
      const result = await acceptServiceAgreementAndNotifyAction(agreement.portalToken, signature);
  ...
  return <div className="space-y-3">
    <Link href={`/agreement/${agreement.portalToken}/document`} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#2d7dff]/30 px-4 py-3 text-sm text-[#d9fbff]"><FileText className="h-4 w-4" />Open agreement</Link>
    <label className="block space-y-2"><span className="text-sm text-zinc-300">Customer signature</span><input value={signature} onChange={(e) => setSignature(e.target.value)} disabled={accepted} placeholder="Type signer name" className="w-full rounded-xl border border-[#2d7dff]/25 bg-black px-4 py-3 text-white disabled:opacity-60" /></label>
    {error ? <p className="text-xs text-rose-300">{error}</p> : null}
    <button type="button" onClick={accept} disabled={pending || accepted} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-500/35 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-100 disabled:opacity-60"><BadgeCheck className="h-4 w-4" />{accepted ? "Plan accepted" : "Sign & accept plan"}</button>
  </div>;
}
