...
export function ManagerEstimateActions({ invoiceId, portalToken, invoiceNumber, customerEmail, customerPhone, status, paymentStatus }: { invoiceId: string; portalToken: string; invoiceNumber: string; customerEmail: string | null; customerPhone: string | null; status: InvoiceStatus; paymentStatus: PaymentStatus }) {
  ...
  const emailCustomer = () => { if (!customerEmail) return; const href = `mailto:${encodeURIComponent(customerEmail)}?subject=${encodeURIComponent(`Chill Bros ${invoiceNumber}`)}&body=${encodeURIComponent(`Review your Chill Bros estimate/invoice here: ${getPortalUrl()}`)}`; window.location.href = href; };
  const textCustomer = () => { if (!customerPhone) return; const href = `sms:${customerPhone.replace(/[^+\d]/g, "")}?&body=${encodeURIComponent(`Chill Bros ${invoiceNumber}: ${getPortalUrl()}`)}`; window.location.href = href; };
