...
import { Box, Rotate3D, Search, ShieldAlert, Wrench } from "lucide-react";

import { Training3DModal } from "@/components/training-3d-modal";
...
export function TrainingBibleLibrary({ cases }: { cases: TrainingCase[] }) {
  ...
  return (
    <>
      <div className="grid w-full min-w-0 gap-4 overflow-x-hidden xl:grid-cols-[0.78fr_1.22fr]">
        <section className="w-full min-w-0 overflow-hidden rounded-2xl border border-[#2d7dff]/30 bg-black/30 p-4 sm:p-5">
          <div className="relative min-w-0">
            ...
            <input aria-label="Search Chill Bro Bible cases" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search symptom, reading, component, code..." className="w-full min-w-0 rounded-xl border border-[#2d7dff]/30 bg-[#020407] py-3 pl-10 pr-3 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-[#8ffafa]/60" />
          </div>

          <div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            {categories.map((item, index) => {
              const spanLastMobileRow = categories.length % 2 === 1 && index === categories.length - 1;
              return <button key={item} type="button" onClick={() => { setCategory(item); setModelOpen(false); }} className={`${spanLastMobileRow ? "col-span-2" : ""} min-h-10 w-full min-w-0 rounded-xl border px-3 py-2 text-center text-xs leading-tight transition sm:min-h-0 sm:w-auto sm:rounded-full sm:py-1.5 ${category === item ? "border-[#8ffafa]/70 bg-[#2d7dff]/15 text-white" : "border-[#2d7dff]/25 text-zinc-400 hover:border-[#8ffafa]/35"}`}>{item}</button>;
            })}
          </div>

          <div className="mt-4 grid w-full min-w-0 gap-2">
            {filtered.map((item) => <button key={item.id} type="button" onClick={() => { setSelectedId(item.id); setModelOpen(false); }} className={`w-full min-w-0 rounded-xl border p-3 text-left transition ${selected.id === item.id ? "border-[#8ffafa]/55 bg-[#2d7dff]/12 shadow-[0_0_12px_rgba(45,125,255,0.12)]" : "border-[#2d7dff]/20 bg-black/20 hover:border-[#2d7dff]/45"}`}><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8ffafa]">{item.category}</p><span className="inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.12em] text-[#bafcfc]"><Rotate3D className="h-3 w-3" />3D</span></div><p className="mt-1 break-words text-sm font-semibold text-white">{item.title}</p><p className="mt-1 break-words text-xs leading-5 text-zinc-500">{item.equipment}</p></button>)}
        ...
        <section className="min-w-0 space-y-4">
          ...
          <div className="w-full min-w-0 rounded-2xl border border-[#2d7dff]/40 bg-gradient-to-br from-[#07152c]/90 via-black/35 to-black/45 p-4 shadow-[0_0_22px_rgba(45,125,255,0.12)] sm:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2"><Box className="h-4 w-4 text-[#8ffafa]" /><h3 className="font-semibold text-white">Interactive 3D component walkthrough</h3></div>
                <p className="mt-2 max-w-2xl break-words text-sm leading-6 text-zinc-400">Open a rotatable service-training schematic for this equipment family. It maps the same components used by the Bible case, while the field diagnosis still depends on the actual unit readings and OEM documentation.</p>
              </div>
              <span className="rounded-full border border-emerald-400/25 bg-emerald-400/5 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-emerald-200">3D ready</span>
            </div>

            <button type="button" onClick={() => setModelOpen(true)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#8ffafa]/45 bg-[#2d7dff]/12 px-4 py-3 text-sm font-semibold text-white shadow-[0_0_14px_rgba(45,125,255,0.18)] transition hover:bg-[#2d7dff]/22 sm:w-auto"><Rotate3D className="h-5 w-5 text-[#8ffafa]" />Open 3D training model</button>
      ...
      <Training3DModal open={modelOpen} onClose={() => setModelOpen(false)} visual={selected.visual} components={selected.components} />
    </>
  );
}
