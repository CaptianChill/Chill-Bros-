...
export function DocumentToolbar({ invoiceNumber, returnHref, backLabel = "Back to estimate" }: { invoiceNumber: string; returnHref: string; backLabel?: string }) {
  ...
  return <div className="print:hidden sticky top-2 z-20 mb-4 rounded-2xl border border-[#2d7dff]/30 bg-[#020407]/95 p-2 shadow-lg backdrop-blur"><div className="flex flex-wrap items-center justify-center gap-2"><button type="button" onClick={goBack} className="inline-flex items-center gap-1.5 rounded-xl border border-[#8ffafa]/35 bg-[#2d7dff]/10 px-3 py-2 text-xs font-medium text-[#d9fbff]"><ArrowLeft className="h-4 w-4" />{backLabel}</button><button type="button" onClick={() => window.print()} className="inline-flex items-center gap-1.5 rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-xs text-white"><Printer className="h-4 w-4" />Print / PDF</button><button type="button" onClick={copy} className="inline-flex items-center gap-1.5 rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-xs text-white"><Copy className="h-4 w-4" />Copy link</button><button type="button" onClick={email} className="inline-flex items-center gap-1.5 rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-xs text-white"><Mail className="h-4 w-4" />Email</button><button type="button" onClick={share} className="inline-flex items-center gap-1.5 rounded-xl border border-[#2d7dff]/25 px-3 py-2 text-xs text-white"><Share2 className="h-4 w-4" />Share</button></div>{message ? <p className="mt-2 text-center text-xs text-[#bafcfc]">{message}</p> : null}</div>;
}
