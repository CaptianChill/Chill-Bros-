...
type InventoryUsageRow = {
  ...
  job: { id: string; equipment_id: string | null; created_at: string } | { id: string; equipment_id: string | null; created_at: string }[] | null;
};
type InventoryPartRow = {
  id: string; name: string; part_number: string;
  default_cost: number | string | null; retail_price: number | string | null; stock: number | string | null; updated_at: string;
};
...
export async function getInventoryIntelligence(){
  ...
  for(const row of (usage??[]) as InventoryUsageRow[]){const job=Array.isArray(row.job)?row.job[0]:row.job;const x=used.get(row.part_id)??{qty:0,jobs:new Set<string>(),equipment:new Set<string>()};x.qty+=Number(row.quantity??0);if(job?.id)x.jobs.add(job.id);if(job?.equipment_id)x.equipment.add(job.equipment_id);used.set(row.part_id,x);}
  const rows=((parts??[]) as InventoryPartRow[]).map((p)=>{const u=used.get(p.id)??{qty:0,jobs:new Set<string>(),equipment:new Set<string>()};const cost=Number(p.default_cost??0),retail=Number(p.retail_price??0),margin=retail>0?((retail-cost)/retail)*100:0;const reorderPoint=Math.max(2,Math.ceil(u.qty/4));const reorderQty=Math.max(0,reorderPoint*2-Number(p.stock??0));return {id:p.id,name:p.name,partNumber:p.part_number,cost,retail,stock:Number(p.stock??0),margin,usedQty:u.qty,jobCount:u.jobs.size,equipmentCount:u.equipment.size,reorderPoint,reorderQty,lowStock:Number(p.stock??0)<=reorderPoint};});
