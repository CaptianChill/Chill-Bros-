...
export async function getOpenTimesheet(technicianId: string): Promise<OpenTimesheet | null> {
  ...
  const { data, error } = await supabase
    ...
    .select("id, location, clock_in_at, job_id")
...
export async function getTimesheetHistory(limit = 100): Promise<TimesheetHistoryRow[]> {
  ...
  const { data, error } = await supabase
    .from("chillbros_timesheets")
    .select("id, location, clock_in_at, clock_out_at, labor_hours, drive_hours, tech:chillbros_profiles(full_name)")
    .order("clock_in_at", { ascending: false })
    .limit(limit);
  if (error || !data) return [];
  return data.map((row) => {
    ...
    return {
      ...
      driveHours: Number(row.drive_hours ?? 0),
    };
  });
}
