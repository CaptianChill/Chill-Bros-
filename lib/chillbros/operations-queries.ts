...
export type DispatchJob = { id: string; customerId: string; customerName: string; assignedTechId: string | null; assignedTechName: string | null; status: JobStatus; location: string | null; scope: string | null; workPerformed: string | null; scheduledWindow: string | null; createdAt: string };
export type ActiveTechnician = { id: string; fullName: string };
export type OpenTimesheet = { id: string; location: string | null; clockInAt: string; jobId: string | null };
...
export async function getOpenTimesheet(technicianId: string): Promise<OpenTimesheet | null> {
  ...
  return { id: data.id, location: data.location, clockInAt: data.clock_in_at, jobId: data.job_id };
}
