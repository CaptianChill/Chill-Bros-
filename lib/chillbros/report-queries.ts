...
export async function getOperationsReport(): Promise<OperationsReport> {
  ...
  return {
    ...
    invoices: { total: invoiceRows.length, awaitingApproval: invoiceRows.filter((i) => i.status === "awaiting_approval").length, approved: invoiceRows.filter((i) => i.status === "approved").length, paid: invoiceRows.filter((i) => i.paymentStatus === "paid").length, outstanding: invoiceRows.filter((i) => i.status === "approved" && i.paymentStatus !== "paid").length, paidRevenue: invoiceRows.filter((i) => i.paymentStatus === "paid").reduce((sum, i) => sum + i.total, 0), outstandingValue: invoiceRows.filter((i) => i.status === "approved" && i.paymentStatus !== "paid").reduce((sum, i) => sum + i.total, 0) },
