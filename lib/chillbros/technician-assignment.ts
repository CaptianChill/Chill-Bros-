...
export async function getAssignedFieldJobsForTechnician(profile: { id:string; email:string }, limit=250): Promise<DispatchJob[]> {
  const supabase=createServiceRoleClient();
  const email=String(profile.email||"").trim().toLowerCase();
  const candidateIds=new Set<string>([profile.id]);
  if(email){
    const {data:matches}=await supabase.from("chillbros_profiles").select("id,email,role,status").ilike("email",email).eq("status","active").in("role",["technician","manager"]);
    for(const row of matches??[]) candidateIds.add(row.id);
  }
  const {data,error}=await supabase.from("chillbros_jobs").select("id,customer_id,assigned_tech_id,status,location,scope,work_performed,scheduled_window,created_at,customer:chillbros_customers(name),tech:chillbros_profiles(full_name)").in("assigned_tech_id",Array.from(candidateIds)).in("status",FIELD_VISIBLE as unknown as string[]).is("archived_at",null).order("created_at",{ascending:false}).limit(limit);
