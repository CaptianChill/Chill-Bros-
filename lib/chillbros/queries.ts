...
export async function getStaffAccounts(): Promise<StaffAccount[]> {
  ...
  const { data: profiles, error } = await supabase
    ...
    .select("id, full_name, email, role, status, phone, last_clock_event")
  ...
  for (const job of jobCounts ?? []) {
    if (!job.assigned_tech_id) continue;
    countByTech.set(job.assigned_tech_id, (countByTech.get(job.assigned_tech_id) ?? 0) + 1);
  }
  ...
  return profiles.map((profile) => ({
    ...
    phone: profile.phone,
    lastClockEvent: profile.last_clock_event,
    assignedJobs: countByTech.get(profile.id) ?? 0,
  }));
}
...
export async function getCustomers(): Promise<Customer[]> {
  ...
  const { data: history } = await supabase
    .from("chillbros_customer_service_history")
    .select("customer_id, note, occurred_on")
    .order("occurred_on", { ascending: false });

  const historyByCustomer = new Map<string, string[]>();
  for (const entry of history ?? []) {
    const list = historyByCustomer.get(entry.customer_id) ?? [];
    list.push(entry.note);
    historyByCustomer.set(entry.customer_id, list);
  }
...
export async function getPartsCatalog(): Promise<PartsCatalogItem[]> {
  ...
  const { data, error } = await supabase
    ...
    .select("id, name, part_number, default_cost, retail_price, stock")
  ...
  return data.map((part) => ({
    ...
    defaultCost: Number(part.default_cost),
    retailPrice: Number(part.retail_price),
    stock: part.stock,
  }));
}
...
export async function getEmailLog(limit = 20): Promise<EmailLogEntry[]> {
  const supabase = createServiceRoleClient();
  const { data, error } = await supabase
    .from("chillbros_email_log")
    .select("id, subject, recipients, status, created_at")
    .order("created_at", { ascending: false })
    .limit(limit);
  if (error || !data) return [];
...
export async function getJob(jobId: string): Promise<Job | null> {
  ...
  const { data: job, error } = await supabase
    .from("chillbros_jobs")
    .select(
      "id, customer_id, assigned_tech_id, status, location, scope, work_performed, labor_hours, drive_hours, scheduled_window, customer:chillbros_customers(name), tech:chillbros_profiles(full_name)",
    )
    .eq("id", jobId)
    .maybeSingle();
  ...
  const [{ data: partsRows }, { data: photoRows }] = await Promise.all([
    supabase
      ...
      .select("id, quantity, part:chillbros_parts_catalog(id, name, part_number, retail_price)")
      .eq("job_id", jobId),
  ...
  return {
    ...
    laborHours: Number(job.labor_hours),
    driveHours: Number(job.drive_hours),
    ...
    parts: (partsRows ?? []).map((row) => {
      ...
      return {
        ...
        partNumber: part?.part_number ?? "",
        retailPrice: Number(part?.retail_price ?? 0),
        quantity: row.quantity,
      };
    }),
    beforePhotos: (photoRows ?? [])
      .filter((p) => p.phase === "before")
      .map((p) => ({ id: p.id, storagePath: p.storage_path, caption: p.caption, url: signedUrlByPath.get(p.storage_path) ?? null })),
    afterPhotos: (photoRows ?? [])
...
export async function getInvoiceByToken(token: string): Promise<Invoice | null> {
  ...
  const { data: lineItems } = await supabase
    .from("chillbros_invoice_line_items")
    .select("id, label, amount")
  ...
  return {
    ...
    notes: invoice.notes,
    lineItems: (lineItems ?? []).map((item) => ({ id: item.id, label: item.label, amount: Number(item.amount) })),
  };
}
...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  ...
  const [{ count: openJobs }, { count: approvalsToday }, { count: lowStockParts }, { count: emailEventsToday }] = await Promise.all([
    ...
    supabase.from("chillbros_parts_catalog").select("id", { count: "exact", head: true }).lt("stock", 5),
  ...
  return {
    ...
    emailEventsToday: emailEventsToday ?? 0,
  };
}
