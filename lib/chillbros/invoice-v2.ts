...
export async function getInvoiceV2ByToken(token: string): Promise<DetailedInvoice | null> {
  ...
  const { data: lineItems } = await supabase.from("chillbros_invoice_line_items").select("id, label, description, quantity, unit_price, amount").eq("invoice_id", invoice.id).order("sort_order", { ascending: true });
  const customer = Array.isArray(invoice.customer) ? invoice.customer[0] : invoice.customer;
  return { id: invoice.id, invoiceNumber: invoice.invoice_number, portalToken: invoice.portal_token, status: invoice.status, customerId: invoice.customer_id, customerName: customer?.name ?? "Unknown customer", jobId: invoice.job_id, signatureName: invoice.signature_name, signedAt: invoice.signed_at, paymentMethod: invoice.payment_method, paymentStatus: invoice.payment_status, notes: invoice.notes, discountType: invoice.discount_type, discountValue: Number(invoice.discount_value ?? 0), discountAmount: Number(invoice.discount_amount ?? 0), downPaymentType: invoice.down_payment_type, downPaymentValue: Number(invoice.down_payment_value ?? 0), downPaymentAmount: Number(invoice.down_payment_amount ?? 0), lineItems: (lineItems ?? []).map((item) => ({ id: item.id, label: item.label, description: item.description, quantity: Number(item.quantity ?? 1), unitPrice: Number(item.unit_price ?? item.amount), amount: Number(item.amount) })) };
}
...
export function invoiceTotals(invoice: DetailedInvoice) {
  ...
  const total = Math.max(0, subtotal - invoice.discountAmount);
  const balanceAfterDownPayment = Math.max(0, total - invoice.downPaymentAmount);
  return { subtotal, total, balanceAfterDownPayment };
}
